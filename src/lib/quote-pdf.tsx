/* eslint-disable @typescript-eslint/no-require-imports */
"use server";

export interface QuoteData {
  id: string;
  clientFirstName: string;
  clientLastName: string;
  clientEmail: string;
  clientPhone: string;
  clientAddress?: string;
  clientCity?: string;
  clientPostalCode?: string;
  macModel: string;
  serialNumber?: string;
  faultType: string;
  faultDescription?: string;
  estimatedCost: number; // prix HT
  createdAt?: Date;
  token: string;
}

const MAC_PLACE = {
  name: "Mac Place",
  address: "5, rue Paul Vaillant Couturier",
  city: "94700 Maisons Alfort",
  phone: "07 82 71 21 23",
  email: "contact@macplace.fr",
};

const LOGO_BASE64 = "";

function formatPrice(n: number) {
  return n.toFixed(2).replace(".", ",") + " €";
}

function formatDate(d: Date) {
  return d.toLocaleDateString("fr-FR", { day: "2-digit", month: "2-digit", year: "numeric" });
}

export async function generateQuotePDF(data: QuoteData): Promise<Buffer> {
  const {
    Document,
    Page,
    Text,
    View,
    Image,
    StyleSheet,
    renderToBuffer,
  } = require("@react-pdf/renderer");

  const React = require("react");

  const htPrice = data.estimatedCost;
  const tva = Math.round(htPrice * 0.2 * 100) / 100;
  const ttcPrice = Math.round((htPrice + tva) * 100) / 100;

  const quoteDate = data.createdAt ? new Date(data.createdAt) : new Date();
  const quoteNumber = data.id.slice(0, 8).toUpperCase();

  const styles = StyleSheet.create({
    page: { fontFamily: "Helvetica", fontSize: 9, padding: 40, color: "#1d1d1f", backgroundColor: "#ffffff" },
    // Header
    header: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 30 },
    logo: { width: 70, height: 70 },
    headerRight: { alignItems: "flex-end" },
    title: { fontSize: 24, fontFamily: "Helvetica-Bold", color: "#000000", marginBottom: 4 },
    quoteRef: { fontSize: 9, color: "#555", marginBottom: 2 },
    // Separator
    separator: { borderBottomWidth: 1, borderBottomColor: "#e0e0e0", marginVertical: 16 },
    // Info blocks
    infoRow: { flexDirection: "row", justifyContent: "space-between", marginBottom: 20 },
    infoBlock: { width: "48%" },
    infoLabel: { fontSize: 7, fontFamily: "Helvetica-Bold", color: "#888", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 },
    infoText: { fontSize: 9, color: "#1d1d1f", lineHeight: 1.6 },
    infoTextBold: { fontSize: 10, fontFamily: "Helvetica-Bold", color: "#000", marginBottom: 2 },
    // Table
    table: { marginTop: 16 },
    tableHeader: { flexDirection: "row", backgroundColor: "#000000", padding: 8 },
    tableHeaderText: { color: "#ffffff", fontFamily: "Helvetica-Bold", fontSize: 8 },
    tableRow: { flexDirection: "row", borderBottomWidth: 1, borderBottomColor: "#f0f0f0", paddingVertical: 10, paddingHorizontal: 8 },
    tableRowAlt: { flexDirection: "row", borderBottomWidth: 1, borderBottomColor: "#f0f0f0", paddingVertical: 10, paddingHorizontal: 8, backgroundColor: "#fafafa" },
    colDesc: { flex: 3 },
    colQty: { flex: 1, textAlign: "center" },
    colPrice: { flex: 1, textAlign: "right" },
    cellText: { fontSize: 9, color: "#333" },
    cellTextBold: { fontSize: 9, fontFamily: "Helvetica-Bold", color: "#111" },
    cellTextSmall: { fontSize: 8, color: "#888", marginTop: 2 },
    // Totals
    totalsBlock: { alignItems: "flex-end", marginTop: 16 },
    totalRow: { flexDirection: "row", width: 220, justifyContent: "space-between", paddingVertical: 4 },
    totalLabel: { fontSize: 9, color: "#555" },
    totalValue: { fontSize: 9, color: "#333" },
    totalTTCRow: { flexDirection: "row", width: 220, justifyContent: "space-between", backgroundColor: "#000", paddingVertical: 8, paddingHorizontal: 10, marginTop: 4 },
    totalTTCLabel: { fontSize: 11, fontFamily: "Helvetica-Bold", color: "#fff" },
    totalTTCValue: { fontSize: 11, fontFamily: "Helvetica-Bold", color: "#fff" },
    // Guarantee
    guaranteeBox: { marginTop: 24, backgroundColor: "#f5f5f7", borderRadius: 6, padding: 12, flexDirection: "row", alignItems: "flex-start" },
    guaranteeText: { fontSize: 9, color: "#424245", lineHeight: 1.6 },
    guaranteeBold: { fontFamily: "Helvetica-Bold", color: "#000" },
    // Footer
    footer: { position: "absolute", bottom: 30, left: 40, right: 40, borderTopWidth: 1, borderTopColor: "#e0e0e0", paddingTop: 10, flexDirection: "row", justifyContent: "space-between" },
    footerText: { fontSize: 7, color: "#aaa" },
  });

  const doc = React.createElement(
    Document,
    null,
    React.createElement(
      Page,
      { size: "A4", style: styles.page },

      // HEADER
      React.createElement(
        View,
        { style: styles.header },
        React.createElement(Image, { src: `data:image/png;base64,${LOGO_BASE64}`, style: styles.logo }),
        React.createElement(
          View,
          { style: styles.headerRight },
          React.createElement(Text, { style: styles.title }, "DEVIS"),
          React.createElement(Text, { style: styles.quoteRef }, `N° ${quoteNumber}`),
          React.createElement(Text, { style: styles.quoteRef }, `Date : ${formatDate(quoteDate)}`),
          React.createElement(Text, { style: styles.quoteRef }, `Valable 30 jours`),
        )
      ),

      React.createElement(View, { style: styles.separator }),

      // INFO EMETTEUR / CLIENT
      React.createElement(
        View,
        { style: styles.infoRow },
        // Emetteur
        React.createElement(
          View,
          { style: styles.infoBlock },
          React.createElement(Text, { style: styles.infoLabel }, "Émetteur"),
          React.createElement(Text, { style: styles.infoTextBold }, MAC_PLACE.name),
          React.createElement(Text, { style: styles.infoText }, MAC_PLACE.address),
          React.createElement(Text, { style: styles.infoText }, MAC_PLACE.city),
          React.createElement(Text, { style: styles.infoText }, MAC_PLACE.phone),
          React.createElement(Text, { style: styles.infoText }, MAC_PLACE.email),
        ),
        // Client
        React.createElement(
          View,
          { style: styles.infoBlock },
          React.createElement(Text, { style: styles.infoLabel }, "Client"),
          React.createElement(Text, { style: styles.infoTextBold }, `${data.clientFirstName} ${data.clientLastName}`),
          data.clientAddress && React.createElement(Text, { style: styles.infoText }, data.clientAddress),
          (data.clientPostalCode || data.clientCity) && React.createElement(Text, { style: styles.infoText }, `${data.clientPostalCode || ""} ${data.clientCity || ""}`.trim()),
          React.createElement(Text, { style: styles.infoText }, data.clientPhone),
          React.createElement(Text, { style: styles.infoText }, data.clientEmail),
        )
      ),

      // TABLE
      React.createElement(
        View,
        { style: styles.table },

        // Header
        React.createElement(
          View,
          { style: styles.tableHeader },
          React.createElement(Text, { style: [styles.tableHeaderText, styles.colDesc] }, "Prestation"),
          React.createElement(Text, { style: [styles.tableHeaderText, styles.colQty] }, "Qté"),
          React.createElement(Text, { style: [styles.tableHeaderText, styles.colPrice] }, "Prix HT"),
        ),

        // Row 1 - Repair
        React.createElement(
          View,
          { style: styles.tableRow },
          React.createElement(
            View,
            { style: styles.colDesc },
            React.createElement(Text, { style: styles.cellTextBold }, `Réparation ${data.macModel}`),
            React.createElement(Text, { style: styles.cellTextSmall }, `Panne : ${data.faultType}${data.faultDescription ? " - " + data.faultDescription : ""}`),
            data.serialNumber && React.createElement(Text, { style: styles.cellTextSmall }, `N° série : ${data.serialNumber}`),
          ),
          React.createElement(Text, { style: [styles.cellText, styles.colQty] }, "1"),
          React.createElement(Text, { style: [styles.cellText, styles.colPrice] }, formatPrice(htPrice)),
        ),

        // Row 2 - Guarantee
        React.createElement(
          View,
          { style: styles.tableRowAlt },
          React.createElement(
            View,
            { style: styles.colDesc },
            React.createElement(Text, { style: styles.cellTextBold }, "Garantie"),
            React.createElement(Text, { style: styles.cellTextSmall }, "12 mois pièces et main d'œuvre"),
          ),
          React.createElement(Text, { style: [styles.cellText, styles.colQty] }, "1"),
          React.createElement(Text, { style: [styles.cellText, styles.colPrice] }, "Incluse"),
        ),
      ),

      // TOTALS
      React.createElement(
        View,
        { style: styles.totalsBlock },
        React.createElement(
          View,
          { style: styles.totalRow },
          React.createElement(Text, { style: styles.totalLabel }, "Sous-total HT"),
          React.createElement(Text, { style: styles.totalValue }, formatPrice(htPrice)),
        ),
        React.createElement(
          View,
          { style: styles.totalRow },
          React.createElement(Text, { style: styles.totalLabel }, "TVA (20%)"),
          React.createElement(Text, { style: styles.totalValue }, formatPrice(tva)),
        ),
        React.createElement(
          View,
          { style: styles.totalTTCRow },
          React.createElement(Text, { style: styles.totalTTCLabel }, "TOTAL TTC"),
          React.createElement(Text, { style: styles.totalTTCValue }, formatPrice(ttcPrice)),
        ),
      ),

      // GUARANTEE BOX
      React.createElement(
        View,
        { style: styles.guaranteeBox },
        React.createElement(
          Text,
          { style: styles.guaranteeText },
          React.createElement(Text, { style: styles.guaranteeBold }, "✓ Garantie 12 mois  "),
          "Ce devis inclut une garantie de 12 mois sur les pièces remplacées et la main d'œuvre. Devis valable 30 jours à compter de la date d'émission. Paiement à réception de la réparation."
        ),
      ),

      // FOOTER
      React.createElement(
        View,
        { style: styles.footer },
        React.createElement(Text, { style: styles.footerText }, `${MAC_PLACE.name} — ${MAC_PLACE.address}, ${MAC_PLACE.city}`),
        React.createElement(Text, { style: styles.footerText }, `${MAC_PLACE.email} — ${MAC_PLACE.phone}`),
      ),
    )
  );

  const buffer = await renderToBuffer(doc);
  return Buffer.from(buffer);
}
